...
let featureItems = Array.isArray(initialFeatureItems)
    ? initialFeatureItems.map(function (feature) {
        return {
            ...
            prefix: String((feature && feature.prefix) || 'emoji').trim() || 'emoji',
...
async function renderFeatureSuggestions(query) {
    ...
    matches.forEach(function (feature) {
        ...
        // build icon node differently for emoji vs icon font
        let iconHtml = '';
        if (feature.prefix === 'emoji') {
            iconHtml = '<span class="feature-suggestion-icon">' + (feature.icon || '') + '</span>';
        } else {
            iconHtml = '<span class="feature-suggestion-icon"><i class="' + feature.prefix + ' ' + feature.icon + '"></i></span>';
        }
...
function addFeature(feature) {
    ...
    const prefix = String((feature && feature.prefix) || 'fas').trim() || 'fas';
