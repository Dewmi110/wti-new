<x-layout bodyClass="g-sidenav-show  bg-gray-200">
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        <x-navbars.navs.auth titlePage="Edit Destination"></x-navbars.navs.auth>
        <div class="container-fluid py-4">
            <div class="row">
                <div class="col-lg-6 col-md-8 mx-auto">
                    <div class="card">
                        <div class="card-header pb-0 px-3 d-flex align-items-center justify-content-between">
                            <h6 class="mb-0">Edit Destination</h6>
                            <a href="{{ route('admin.destinations.index') }}" class="btn btn-outline-primary btn-sm mb-0">Back</a>
                        </div>
                        <div class="card-body pt-4 p-3">
                            <form action="{{ route('admin.destinations.update', $item) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="input-group input-group-outline mt-3 is-filled">
                                    ...
                                    <select name="country_id" class="form-control">
                                        @foreach($countries as $c)
                                            <option value="{{ $c->id }}" {{ $item->country_id == $c->id ? 'selected' : '' }}>{{ $c->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="mt-4">
                                    ...
                                    <div class="image-preview-box" id="imagePreview" @if(!$item->image)style="display: none;"@endif>
                                        <img id="previewImage" src="@if($item->image){{ asset('storage/' . $item->image) }}@endif" alt="Preview" class="preview-img">
